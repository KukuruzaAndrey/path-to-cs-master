...
fun remove_duplicates (months: int list) =
    let
        fun has (num: int, numbers: int list) =
            if null numbers
            then false
            else num = hd numbers orelse has (num, tl numbers)
    in
        if null months 
        then []
        else
            if has(hd months, tl months)
...
years. Leap years are years that are either divisible by 400 or divisible by 4 but not divisible by 100. (Do not worry about days possibly lost in the conversion to the Gregorian calendar in the Late 1500s.) *)
fun reasonable_date (date: int*int*int) =
    ...
    in
        ...
        (day <= get_nth(month_day_counts, month) orelse
        (is_leap_year(year) andalso month = 2 andalso day = 29))
    end
